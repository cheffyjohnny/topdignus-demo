...
import { saveDuctSale, saveDuctPurchase } from '@/lib/ecount'
...
export async function PATCH(
  ...
  if (body._full) {
    // vendor는 프론트 정규화 필드 (실제 컬럼명: customer_name), order_client/id/created_at은 duct_orders에 없는 컬럼
  ...
  if (body.status === '납품') {
    ...
    try { saleResult = await saveDuctSale(orderData) } catch (e: any) {
      ecountError = e.message ?? 'ECOUNT 오류'
      console.error('[ecount] 덕트 판매입력 실패:', ecountError)
    }
    try { purchaseResult = await saveDuctPurchase(orderData) } catch (e: any) {
      if (!ecountError) ecountError = e.message ?? 'ECOUNT 오류'
      console.error('[ecount] 덕트 구매입력 실패:', e.message)
    }
