...
export default function QuoteGroupDetailPage() {
  ...
  async function handleDelete() {
    ...
    toast.success('그룹 견적서가 삭제되었습니다.')
    router.push('/dashboard/quotes')
  }

  if (loading) return <div className="text-center py-20 text-sm text-gray-400">불러오는 중...</div>
  if (!group)  return <div className="text-center py-20 text-sm text-red-400">그룹 견적서를 찾을 수 없습니다.</div>
    ...
    <div className="max-w-5xl mx-auto space-y-5 pb-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-gray-900">복합 견적서</h1>
              <span className="text-xs font-medium px-2 py-0.5 rounded-full border bg-purple-50 text-purple-700 border-purple-200">배관+덕트</span>
            </div>
            <p className="text-sm text-gray-500 mt-0.5">{group.vendor} · {group.project || '현장명 없음'}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        <div className="p-5 grid grid-cols-2 gap-4 text-sm">
          <div><span className="text-gray-400 text-xs block mb-0.5">발주의뢰처</span><span className="font-medium">{group.vendor}</span></div>
          <div><span className="text-gray-400 text-xs block mb-0.5">현장명</span><span>{group.project || '-'}</span></div>
          <div><span className="text-gray-400 text-xs block mb-0.5">작성일</span><span>{fmtDate(group.order_date)}</span></div>
          <div><span className="text-gray-400 text-xs block mb-0.5">작성자</span><span>{group.author || '-'}</span></div>
          {group.notes && <div className="col-span-2"><span className="text-gray-400 text-xs block mb-0.5">비고</span><span className="whitespace-pre-wrap">{group.notes}</span></div>}
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        <div className="px-5 py-3.5 border-b border-gray-200 flex items-center gap-2">
          <span className="w-1 h-4 rounded-full flex-shrink-0 bg-purple-400" />
          <h2 className="font-semibold text-gray-800 text-sm">하위 견적서</h2>
          <span className="text-xs text-gray-400 ml-auto">{allSubs.length}건</span>
        </div>
        <div className="p-4 flex flex-wrap gap-3">
          {group.pipe_quotes.map(sub => (
            <button key={sub.id} onClick={() => router.push(`/dashboard/pipe-quotes/${sub.id}?from=/dashboard/quotes/groups/${id}`)}
              ...
              <span className="text-xs font-medium text-blue-700">배관</span>
              <span className="text-xs text-blue-500">{sub.manufacturer}</span>
              <span className={`text-xs font-medium px-1.5 py-0.5 rounded-full border ${STATUS_COLORS[sub.status]}`}>{sub.status}</span>
          ...
          {group.duct_quotes.map(sub => (
            <button key={sub.id} onClick={() => router.push(`/dashboard/duct-quotes/${sub.id}?from=/dashboard/quotes/groups/${id}`)}
              ...
              <span className="text-xs font-medium text-orange-700">덕트</span>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        {activeSubs.length === 0 ? (
          <div className="px-5 py-10 text-center text-sm text-gray-400">{activeTab} 견적서 없음</div>
        ) : activeSubs.map(sub => (
          <div key={sub.id} className="border-b border-gray-100 last:border-b-0">
            <div className="px-4 py-3 flex items-center justify-between bg-gray-50/50">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-medium px-1.5 py-0.5 rounded-full border ${STATUS_COLORS[sub.status]}`}>{sub.status}</span>
                {sub.delivery_date && <span className="text-xs text-gray-400">유효기간: {fmtDate(sub.delivery_date)}</span>}
              </div>
              <button onClick={() => router.push(activeTab === '배관' ? `/dashboard/pipe-quotes/${sub.id}?from=/dashboard/quotes/groups/${id}` : `/dashboard/duct-quotes/${sub.id}?from=/dashboard/quotes/groups/${id}`)}
                ...
                상세 보기 <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </button>
            </div>
            {activeTab === '배관' ? (
              <div className="overflow-x-auto">
                <table className="w-full text-xs min-w-[500px]">
                  <thead className="bg-gray-50 text-gray-400 border-b border-gray-100"><tr>
                    <th className="relative group/th px-4 py-2 text-left select-none" style={{width:pipeColW.name}}>품목명{rhPipe('name')}</th>
                    <th className="relative group/th px-4 py-2 text-left select-none" style={{width:pipeColW.spec}}>규격{rhPipe('spec')}</th>
                    <th className="relative group/th px-4 py-2 text-right select-none" style={{width:pipeColW.qty}}>수량{rhPipe('qty')}</th>
                    <th className="relative group/th px-4 py-2 text-right select-none" style={{width:pipeColW.price}}>판매가{rhPipe('price')}</th>
                    <th className="relative group/th px-4 py-2 text-right select-none" style={{width:pipeColW.amount}}>공급가액{rhPipe('amount')}</th>
                  </tr></thead>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs min-w-[500px]">
                  <thead className="bg-gray-50 text-gray-400 border-b border-gray-100"><tr>
                    <th className="relative group/th px-4 py-2 text-center select-none" style={{width:ductColW.type}}>품명{rhDuct('type')}</th>
                    <th className="relative group/th px-4 py-2 text-right select-none" style={{width:ductColW.w}}>가로{rhDuct('w')}</th>
                    <th className="relative group/th px-4 py-2 text-right select-none" style={{width:ductColW.h}}>세로{rhDuct('h')}</th>
                    <th className="relative group/th px-4 py-2 text-right select-none" style={{width:ductColW.qty}}>수량{rhDuct('qty')}</th>
                    <th className="relative group/th px-4 py-2 text-right select-none" style={{width:ductColW.amount}}>공급가액{rhDuct('amount')}</th>
                  </tr></thead>
                  <tbody className="divide-y divide-gray-50">
                    {(sub.items ?? []).map((it: any, i: number) => (
                      <tr key={i} className="hover:bg-gray-50/50">
                        <td className="px-4 py-2 text-center">
                          <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${it.type==='입상'?'bg-blue-50 text-blue-700':it.type==='벽체'?'bg-violet-50 text-violet-700':'bg-orange-50 text-orange-700'}`}>{it.type}</span>
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-900">그룹 견적서 삭제</p>
                <p className="text-sm text-gray-500 mt-0.5">하위 견적서는 그룹에서 분리됩니다.</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg px-4 py-3 text-sm space-y-1">
              <p><span className="text-gray-400 w-16 inline-block">업체</span><span className="font-medium">{group.vendor}</span></p>
              <p><span className="text-gray-400 w-16 inline-block">현장명</span><span>{group.project || '-'}</span></p>
            </div>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setShowDelete(false)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 cursor-pointer">취소</button>
