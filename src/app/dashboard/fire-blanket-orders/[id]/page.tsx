...
export default function FireBlanketOrderDetailPage() {
  ...
  async function toggleNoInvoice() {
    ...
    toast.success(next ? 'Set to no invoice.' : 'Set to invoice issued.')
  }
  ...
  async function saveEdit() {
    ...
    if (res.ok) { setOrder(editData); setEditMode(false); setEditData(null); toast.success('Saved.') }
    setSaving(false)
  }
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      if (!res.ok) { const dd = await res.json().catch(() => ({})); setPdfError(dd.error ?? `Error ${res.status}`); return }
  ...
  async function handleSendOrder() {
    ...
    } catch { setSendResult('error'); toast.error('An error occurred while sending.', { autoClose: false }) }
    ...
    <>
      <div className="max-w-4xl mx-auto space-y-6 pb-16">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 mt-0.5">{d.vendor} · {d.order_date?.slice(0, 10) ?? '-'} · Fire Blanket</p>
            </div>
          </div>
            ...
            {editMode ? (
              ...
              <>
                <button
                  ...
                  title={order.no_invoice ? 'Switch to invoice issued' : 'Set to no invoice'}
                >
        ...
        {editMode ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <EditCard title="Order Info">
              ...
              <EditRow label="Requested Delivery Date"><input type="date" value={d.delivery_date?.slice(0, 10) ?? ''} onChange={e => setField('delivery_date', e.target.value)} className={INPUT_CLS} /></EditRow>
            ...
            <EditCard title="Delivery Info">
              <EditRow label="Contact Person"><input value={d.contact_name ?? ''} onChange={e => setField('contact_name', e.target.value)} className={INPUT_CLS} /></EditRow>
              <EditRow label="Contact Phone"><input value={d.contact_phone ?? ''} onChange={e => setField('contact_phone', e.target.value)} className={INPUT_CLS} /></EditRow>
              ...
              <EditRow label="Delivery Recipient"><input value={d.delivery_dest ?? ''} onChange={e => setField('delivery_dest', e.target.value)} className={INPUT_CLS} /></EditRow>
            </EditCard>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <InfoCard title="Order Info">
              <InfoRow label="Manufacturer" value={order.manufacturer} />
              <InfoRow label="Requesting Party" value={order.vendor} />
              <InfoRow label="Project" value={order.project} />
              <InfoRow label="Order Date" value={order.order_date?.slice(0, 10)} />
              <InfoRow label="Requested Delivery Date" value={order.delivery_date?.slice(0, 10)} />
              <InfoRow label="Author" value={order.author} />
              <InfoRow label="Notes" value={order.notes} />
            </InfoCard>
            <InfoCard title="Delivery Info">
              <InfoRow label="Contact Person" value={order.contact_name} />
              <InfoRow label="Phone" value={order.contact_phone} />
              <InfoRow label="Delivery Location" value={order.delivery_location} />
              <InfoRow label="Address" value={order.address} />
              <InfoRow label="Delivery Recipient" value={order.delivery_dest} />
            </InfoCard>
          </div>
        )}
        ...
        {!editMode && (() => {
          ...
          return (
            <div className="border border-gray-200 rounded-xl p-4">
              <p className="text-xs font-semibold text-gray-500 mb-3">Attachments ({effectiveUrls.length})</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-500 text-xs border-b border-gray-100"><tr>
                ...
                <th className="px-3 py-2.5 text-left w-40">Item Name</th>
                <th className="px-3 py-2.5 text-left w-32">Spec</th>
                <th className="px-3 py-2.5 text-left w-28">Manufacturer</th>
                <th className="px-3 py-2.5 text-right w-20">Qty (rolls)</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.length === 0 && !editMode && <tr><td colSpan={8} className="px-4 py-6 text-center text-sm text-gray-400">No items</td></tr>}
                {items.map((item, i) => (
                  <tr key={i} className="bg-white">
                    <td className="px-3 py-2.5 text-gray-400 text-xs text-center">{i + 1}</td>
                    {editMode ? (
                      <>
                        <td className="px-2 py-2"><input value={item.name} onChange={e => setItemField(i, 'name', e.target.value)} className={INPUT_CLS} placeholder="Item Name" /></td>
                        <td className="px-2 py-2"><input value={item.spec ?? ''} onChange={e => setItemField(i, 'spec', e.target.value)} className={INPUT_CLS} placeholder="Spec" /></td>
                        <td className="px-2 py-2"><input value={item.manufacturer ?? ''} onChange={e => setItemField(i, 'manufacturer', e.target.value)} className={INPUT_CLS} placeholder="Manufacturer" /></td>
                        ...
                        <td className="px-2 py-2"><input value={item.note ?? ''} onChange={e => setItemField(i, 'note', e.target.value)} className={INPUT_CLS} placeholder="Note" /></td>
                ...
                {!editMode && totalAmount > 0 && <tr className="bg-gray-50 font-semibold"><td colSpan={6} className="px-3 py-2.5 text-right text-xs text-gray-500">Total</td><td className="px-3 py-2.5 text-right text-gray-800 tabular-nums">{totalAmount.toLocaleString('ko-KR')} KRW</td><td /></tr>}
              </tbody>
            </table>
          </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-gray-200">
          ...
          {editMode ? (
            <div className="flex flex-col items-end gap-1">
              ...
              <span className="text-xs text-amber-500">ECOUNT must be updated manually if changed</span>
            </div>
          ) : (
            <span className="text-sm font-medium text-gray-800">{(order.freight ?? 0) > 0 ? `${fmtNum(order.freight)} KRW` : '—'}</span>
          )}
        </div>
        ...
        {!editMode && (() => {
          ...
          return (
            <div className="rounded-xl border border-gray-200 overflow-hidden">
              <div className="bg-gray-50 px-4 py-2.5 text-xs font-medium text-gray-500 border-b border-gray-100">Send History</div>
        ...
        {!editMode && (() => {
          ...
          return (
            <div className="rounded-xl border border-gray-200 px-4 py-3 space-y-2">
              <p className="text-xs font-medium text-gray-500">Status History</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={e => { if (e.target === e.currentTarget) { setShowFreightModal(false); setFreightInput('') } }}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-semibold text-gray-900">Mark as Delivered</p><p className="text-sm text-gray-500 mt-0.5">Enter the freight cost to mark this order delivered.</p></div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Freight Cost (KRW)</label>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden max-h-[90vh] flex flex-col">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
                <div><h2 className="text-base font-bold text-gray-900">Send Purchase Order</h2><p className="text-xs text-gray-400 mt-0.5">Attach a PDF before sending.</p></div>
