...
export default function OrderDetailPage({ type }: { type: OrderType }) {
  ...
  async function updateStatus(status: OrderStatus, freight?: number) {
    ...
    toast.success(`Changed to "${statusLabel(status, ORDER_STATUS_LABEL)}".`)
    if (status === '납품') {
      if (data.ecount === 'ok') toast.success('[ECOUNT] Sale/purchase entry complete', { autoClose: 3000 })
      else if (data.ecount === 'skipped') toast.info('Skipped ECOUNT sale/purchase entry — no matching item code.')
      else if (data.ecount === 'fail') toast.error(`ECOUNT entry failed${data.ecountError ? `\n${data.ecountError}` : ''}`, { autoClose: false })
    }
  }
  ...
  async function toggleNoInvoice() {
    ...
    if (!res.ok) { toast.error('Change failed'); return }
    setOrder(prev => prev ? { ...prev, no_invoice: next } : prev)
    toast.success(next ? 'Set to no invoice.' : 'Changed to invoice required.')
  }
  ...
  async function saveEdit() {
    ...
    if (res.ok) { setOrder(editData); setEditMode(false); setEditData(null); toast.success('Saved successfully.') }
    setSaving(false)
  }
  ...
  async function handleDownloadPdf(mfr?: string) {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json().catch(()=>({})); setPdfError(d.error ?? `Error ${res.status}`); return }
      const blob = await res.blob()
      const match = res.headers.get('Content-Disposition')?.match(/filename\*=UTF-8''(.+)/)
      const filename = match ? decodeURIComponent(match[1]) : 'order.pdf'
      const objUrl = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href = objUrl; a.download = filename; a.click(); URL.revokeObjectURL(objUrl)
    } catch(e) { setPdfError(e instanceof Error ? e.message : 'Unknown error') }
  ...
  async function handleDownloadExcel(mfr?: string) {
    ...
    try {
      ...
      const blob = await res.blob()
      const match = res.headers.get('Content-Disposition')?.match(/filename\*=UTF-8''(.+)/)
      const filename = match ? decodeURIComponent(match[1]) : 'order.xlsx'
      const objUrl = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href = objUrl; a.download = filename; a.click(); URL.revokeObjectURL(objUrl)
    } finally { setDownloading(false) }
  }
  ...
  function openSendDialog(mfr: string) {
    ...
    setEmailBody(`Hello, this is Topdignus.\n\nWe would like to place an order for the '${order?.project ?? ''}' site.\nPlease contact us if there are any issues with the delivery date.\n\nThank you.`)
  }
  ...
  async function handleSendOrder() {
    ...
    try {
      ...
      if (!res.ok) { setSendError(rd.error ?? 'Error'); setSendResult('error'); toast.error(rd.error ?? 'Send error', { autoClose: false }); return }
      setSendResult('success'); toast.success(`Sent to ${recipientEmail}.`)
      const now = new Date().toISOString()
      setOrder(prev => {
        if (!prev) return prev
        return { ...prev, status:'발주', status_history: [...(prev.status_history??[]),
          { type:'send' as const, to:recipientEmail, subject:`[Purchase Order] ${order.project}`, fileUrl:rd.fileUrl, fileName:rd.fileName, at:now },
          { type:'status' as const, value:'발주', at:now }] }
      })
    } catch { setSendResult('error'); toast.error('Error while sending', { autoClose: false }) }
    finally { setSending(false) }
  }

  // ── Render ──
  if (loading) return <div className="flex items-center justify-center h-48 text-gray-400 text-sm">Loading...</div>
  if (!order || !d)  return <div className="text-sm text-red-500 p-6">Order not found.</div>
      ...
      <div className="w-full space-y-6 pb-16 print:hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl font-bold text-gray-900">{d.project || '(No project name)'}</h1>
                {order.order_no && <span className="text-xs font-mono text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{order.order_no}</span>}
                {order.no_invoice && <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-200 text-gray-500 border border-gray-300">No Invoice</span>}
              </div>
              <p className="text-sm text-gray-400 mt-0.5">{d.vendor} · {d.order_date?.slice(0,10) ?? '-'} · {type === 'pipe' ? 'Pipe' : 'Rectangular Duct'}</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            {editMode ? (
              <>
                <button onClick={cancelEdit} className="px-3 py-1.5 text-sm text-gray-600 hover:text-gray-800 transition-colors cursor-pointer">Cancel</button>
                <button onClick={saveEdit} disabled={saving} className="px-4 py-1.5 rounded-lg text-sm font-semibold text-white disabled:opacity-40 cursor-pointer" style={{ backgroundColor: '#014A99' }}>{saving ? 'Saving...' : 'Save'}</button>
              </>
            ) : (
              <>
                <button
                  ...
                  title={order.no_invoice ? 'Switch to invoice required' : 'Set to no invoice'}
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                  {order.no_invoice ? 'Switch to Invoice' : 'Switch to No Invoice'}
                </button>
                ...
                <button onClick={startEdit} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium text-gray-600 border border-gray-300 hover:bg-gray-50 transition-colors cursor-pointer">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536M9 13l6.586-6.586a2 2 0 012.828 0l.172.172a2 2 0 010 2.828L12 16H9v-3z" /></svg>Edit
                </button>
                <select value={order.status} onChange={e => { const s = e.target.value as OrderStatus; if (s === order.status) return; if (s === '납품') { setShowFreightModal(true); return } setPendingStatus(s) }}
                  ...
                  {STATUS_LIST.map(s => <option key={s} value={s} disabled={s === '납품' && order.status !== '발주'}>{statusLabel(s, ORDER_STATUS_LABEL)}</option>)}
                </select>
              </>
            )}
          </div>
        ...
        {editMode ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <EditCard title="Order Info">
              {type === 'pipe' ? (
                <>
                  <EditRow label="Requesting Party"><input value={d.vendor ?? ''} onChange={e => setField('vendor', e.target.value)} className={INPUT_CLS} /></EditRow>
                  <EditRow label="Delivery Dest."><input value={d.delivery_dest ?? ''} onChange={e => setField('delivery_dest', e.target.value)} className={INPUT_CLS} /></EditRow>
                </>
              ) : (
                <>
                  <EditRow label="Manufacturer"><input value={d.manufacturer ?? ''} onChange={e => setField('manufacturer', e.target.value)} className={INPUT_CLS} /></EditRow>
                  <EditRow label="Requesting Party"><input value={d.vendor ?? ''} onChange={e => setField('vendor', e.target.value)} className={INPUT_CLS} /></EditRow>
                </>
              )}
              <EditRow label="Project"><input value={d.project ?? ''} onChange={e => setField('project', e.target.value)} className={INPUT_CLS} /></EditRow>
              {type === 'pipe' && <EditRow label="Manufacturer"><input value={d.manufacturer ?? ''} onChange={e => setField('manufacturer', e.target.value)} className={INPUT_CLS} /></EditRow>}
              <EditRow label="Order Date"><input type="date" value={d.order_date?.slice(0,10) ?? ''} onChange={e => setField('order_date', e.target.value)} className={INPUT_CLS} /></EditRow>
              <EditRow label="Delivery Date"><input type="date" value={d.delivery_date?.slice(0,10) ?? ''} onChange={e => setField('delivery_date', e.target.value)} className={INPUT_CLS} /></EditRow>
              <EditRow label="Author"><input value={d.author ?? ''} onChange={e => setField('author', e.target.value)} className={INPUT_CLS} /></EditRow>
              <EditRow label="Notes"><textarea value={d.notes ?? ''} onChange={e => setField('notes', e.target.value)} rows={3} className={INPUT_CLS + ' resize-y'} /></EditRow>
            </EditCard>
            <EditCard title="Delivery Info">
              <EditRow label="Recipient"><input value={d.contact_name ?? ''} onChange={e => setField('contact_name', e.target.value)} className={INPUT_CLS} /></EditRow>
              <EditRow label="Recipient Phone"><input value={d.contact_phone ?? ''} onChange={e => setField('contact_phone', e.target.value)} className={INPUT_CLS} /></EditRow>
              <EditRow label="Delivery Location"><input value={d.delivery_location ?? ''} onChange={e => setField('delivery_location', e.target.value)} className={INPUT_CLS} /></EditRow>
              <EditRow label="Address"><input value={d.address ?? ''} onChange={e => setField('address', e.target.value)} className={INPUT_CLS} /></EditRow>
              {type === 'duct' && <EditRow label="Delivery Dest."><input value={d.delivery_dest ?? ''} onChange={e => setField('delivery_dest', e.target.value)} className={INPUT_CLS} /></EditRow>}
            </EditCard>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <InfoCard title="Order Info">
              {type === 'pipe' ? (
                <><InfoRow label="Requesting Party" value={order.vendor} /><InfoRow label="Delivery Dest." value={order.delivery_dest} /></>
              ) : (
                <><InfoRow label="Manufacturer" value={order.manufacturer} /><InfoRow label="Requesting Party" value={order.vendor} /></>
              )}
              <InfoRow label="Project" value={order.project} />
              {type === 'pipe' && <InfoRow label="Manufacturer" value={order.manufacturer} />}
              <InfoRow label="Order Date" value={order.order_date?.slice(0,10)} />
              <InfoRow label="Delivery Date" value={order.delivery_date?.slice(0,10)} />
              <InfoRow label="Author" value={order.author} />
              <InfoRow label="Notes" value={order.notes} />
            </InfoCard>
            <InfoCard title="Delivery Info">
              <InfoRow label="Recipient" value={order.contact_name} />
              <InfoRow label="Phone" value={order.contact_phone} />
              <InfoRow label="Delivery Location" value={order.delivery_location} />
              <InfoRow label="Address" value={order.address} />
              {type === 'duct' && <InfoRow label="Delivery Dest." value={order.delivery_dest} />}
            </InfoCard>
          </div>
        )}
        ...
        {!editMode && (() => {
          ...
          return (
            <div className="border border-gray-200 rounded-xl overflow-hidden">
              <p className="text-xs font-semibold text-gray-500 px-4 py-3 border-b border-gray-100">Attachments ({effectiveUrls.length})</p>
          ...
          <div className="rounded-xl border border-orange-200 overflow-hidden">
            <div className="px-5 py-2.5 bg-orange-50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-orange-700">Heat Insulator Calculation Reference</span>
                <span className="flex items-center gap-1.5 text-xs text-orange-600">
                  50T×400 <span className="bg-orange-200 text-orange-900 font-semibold px-1.5 py-0.5 rounded">{insulCalc.rolls50} rolls</span>
                  {insulCalc.rolls25 > 0 && <> · 25T×400 <span className="bg-orange-200 text-orange-900 font-semibold px-1.5 py-0.5 rounded">{insulCalc.rolls25} rolls</span></>}
                </span>
              </div>
              <div className="flex gap-3">
                {insulCalc.wallRows.length > 0 && <button onClick={() => setShowInsulCalcWall(v=>!v)} className="flex items-center gap-1 text-xs text-orange-500 hover:text-orange-700 cursor-pointer">Wall <svg className={`w-3.5 h-3.5 transition-transform ${showInsulCalcWall?'rotate-180':''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></button>}
                {insulCalc.riserRows.length > 0 && <button onClick={() => setShowInsulCalcRiser(v=>!v)} className="flex items-center gap-1 text-xs text-orange-500 hover:text-orange-700 cursor-pointer">Riser <svg className={`w-3.5 h-3.5 transition-transform ${showInsulCalcRiser?'rotate-180':''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></button>}
              </div>
            </div>
            {showInsulCalcWall && insulCalc.wallRows.length > 0 && (
              <div className="border-t-2 border-orange-200">
                <div className="px-5 py-2 bg-orange-100/60 border-b border-orange-200"><span className="text-xs font-bold text-orange-700">Wall</span> <span className="text-xs text-orange-400">50T×400</span></div>
                <div className="px-5 py-3 text-xs text-orange-900">
                  <table className="w-full border-collapse"><thead><tr className="text-orange-500 border-b border-orange-200"><th className="text-left py-1 pr-4">Dimensions</th><th className="text-left py-1 pr-4">Qty</th><th className="text-left py-1 pr-4">Formula</th><th className="text-right py-1">Rolls</th></tr></thead>
                    <tbody>{insulCalc.wallRows.map((r,i)=><tr key={i} className="border-b border-orange-100"><td className="py-1 pr-4">{r.it.width}×{r.it.height}</td><td className="py-1 pr-4">{r.it.quantity}</td><td className="py-1 pr-4 font-mono">perimeter {r.pMm.toLocaleString()}mm×2×2×{r.it.quantity} = {r.mm50.toLocaleString()}mm</td><td className="py-1 text-right">{Math.ceil(r.mm50/3600)} rolls</td></tr>)}</tbody>
                  </table>
                  <div className="pt-1.5 mt-1 border-t border-orange-200 font-semibold text-orange-800">Total: {insulCalc.wallMm50.toLocaleString()}mm ÷ 3,600 = <strong>{Math.ceil(insulCalc.wallMm50/3600)} rolls</strong></div>
                </div>
              </div>
            )}
            {showInsulCalcRiser && insulCalc.riserRows.length > 0 && (
              <div className="border-t-2 border-orange-300">
                <div className="px-5 py-2 bg-amber-50 border-b border-orange-200"><span className="text-xs font-bold text-orange-700">Riser</span> <span className="text-xs text-orange-400">50T×400 · 25T×400</span></div>
                <div className="px-5 py-3 text-xs text-orange-900 space-y-3">
                  <div>
                    <p className="font-semibold mb-1 text-orange-600">Levels 1–2 (50T)</p>
                    <table className="w-full border-collapse"><thead><tr className="text-orange-500 border-b border-orange-200"><th className="text-left py-1 pr-4">Dimensions</th><th className="text-left py-1 pr-4">Qty</th><th className="text-left py-1 pr-4">Formula</th><th className="text-right py-1">Rolls</th></tr></thead>
                      <tbody>{insulCalc.riserRows.map((r,i)=><tr key={i} className="border-b border-orange-100"><td className="py-1 pr-4">{r.it.width}×{r.it.height}</td><td className="py-1 pr-4">{r.it.quantity}</td><td className="py-1 pr-4 font-mono">perimeter {r.pMm.toLocaleString()}mm×2×{r.it.quantity} = {r.mm50.toLocaleString()}mm</td><td className="py-1 text-right">{Math.ceil(r.mm50/3600)} rolls</td></tr>)}</tbody>
                    </table>
                    <div className="pt-1.5 mt-1 border-t border-orange-200 font-semibold text-orange-800">Total: {insulCalc.riserMm50.toLocaleString()}mm ÷ 3,600 = <strong>{Math.ceil(insulCalc.riserMm50/3600)} rolls</strong></div>
                  </div>
                  <div className="border-t border-dashed border-orange-200 pt-3">
                    <p className="font-semibold mb-1 text-orange-600">Levels 1–3 (25T)</p>
                    <table className="w-full border-collapse"><thead><tr className="text-orange-500 border-b border-orange-200"><th className="text-left py-1 pr-4">Dimensions</th><th className="text-left py-1 pr-4">Qty</th><th className="text-left py-1 pr-4">Formula</th><th className="text-right py-1">Rolls</th></tr></thead>
                      <tbody>{insulCalc.riserRows.map((r,i)=><tr key={i} className="border-b border-orange-100"><td className="py-1 pr-4">{r.it.width}×{r.it.height}</td><td className="py-1 pr-4">{r.it.quantity}</td><td className="py-1 pr-4 font-mono">perimeter {r.pMm.toLocaleString()}mm×3×{r.it.quantity} = {r.mm25.toLocaleString()}mm</td><td className="py-1 text-right">{Math.ceil(r.mm25/7200)} rolls</td></tr>)}</tbody>
                    </table>
                    <div className="pt-1.5 mt-1 border-t border-orange-200 font-semibold text-orange-800">Total: {insulCalc.riserMm25.toLocaleString()}mm ÷ 7,200 = <strong>{Math.ceil(insulCalc.riserMm25/7200)} rolls</strong></div>
                  </div>
                </div>
              </div>
            )}
        ...
        {type === 'duct' && isProfire && detailAppliedCalc && detailSalePrice && insulCalc && (
          <div className="rounded-xl border border-blue-200 overflow-hidden">
            <div className="px-5 py-2.5 bg-blue-50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-blue-700">Applied Rate Calculation</span>
                <span className="flex items-center gap-3 text-xs text-blue-600">
                  {detailAppliedCalc.riserTotalM > 0 && <span>Riser <span className="bg-blue-200 text-blue-900 font-semibold px-1.5 py-0.5 rounded">₩{detailAppliedCalc.riserApplied.toLocaleString()}/M</span></span>}
                  {detailAppliedCalc.wallTotalM  > 0 && <span>Wall <span className="bg-blue-200 text-blue-900 font-semibold px-1.5 py-0.5 rounded">₩{detailAppliedCalc.wallApplied.toLocaleString()}/M</span></span>}
                </span>
              </div>
              <button onClick={() => setShowAppliedCalcDetail(v=>!v)} className="text-xs text-blue-500 hover:text-blue-700 cursor-pointer flex items-center gap-1">View Calculation <svg className={`w-3.5 h-3.5 transition-transform ${showAppliedCalcDetail?'rotate-180':''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></button>
            </div>
            {showAppliedCalcDetail && (
              <div className="px-5 py-4 space-y-5 text-xs text-blue-900 border-t border-blue-100">
                <p className="text-blue-500 font-semibold">Formula: ( Sale Price × Total M - Insulator Cost ) / Total M → rounded to nearest 1,000</p>
                {detailAppliedCalc.riserTotalM > 0 && (
                  <div><p className="font-bold text-blue-700 mb-2">Riser</p>
                    <table className="w-full border-collapse"><tbody className="divide-y divide-blue-100">
                      <tr><td className="py-1.5 pr-4 text-blue-500 w-28">Sale Price</td><td className="py-1.5 font-mono">₩{detailSalePrice.riser_sale_price.toLocaleString()}/M</td></tr>
                      <tr><td className="py-1.5 pr-4 text-blue-500">Total M</td><td className="py-1.5 font-mono">{detailAppliedCalc.riserTotalM.toFixed(3)}M</td></tr>
                      <tr><td className="py-1.5 pr-4 text-blue-500">Insulator Cost</td><td className="py-1.5 font-mono">₩{detailAppliedCalc.riserInsulCost.toLocaleString()}</td></tr>
                      <tr className="text-blue-700 font-semibold"><td className="py-1.5 pr-4">Applied Rate</td><td className="py-1.5">₩{detailAppliedCalc.riserApplied.toLocaleString()}/M</td></tr>
                    </tbody></table>
                  </div>
                )}
                {detailAppliedCalc.wallTotalM > 0 && (
                  <div><p className="font-bold text-blue-700 mb-2">Wall</p>
                    <table className="w-full border-collapse"><tbody className="divide-y divide-blue-100">
                      <tr><td className="py-1.5 pr-4 text-blue-500 w-28">Sale Price</td><td className="py-1.5 font-mono">₩{detailSalePrice.wall_sale_price.toLocaleString()}/M</td></tr>
                      <tr><td className="py-1.5 pr-4 text-blue-500">Total M</td><td className="py-1.5 font-mono">{detailAppliedCalc.wallTotalM.toFixed(3)}M</td></tr>
                      <tr><td className="py-1.5 pr-4 text-blue-500">Insulator Cost</td><td className="py-1.5 font-mono">₩{detailAppliedCalc.wallInsulCost.toLocaleString()}</td></tr>
                      <tr className="text-blue-700 font-semibold"><td className="py-1.5 pr-4">Applied Rate</td><td className="py-1.5">₩{detailAppliedCalc.wallApplied.toLocaleString()}/M</td></tr>
                    </tbody></table>
                  </div>
                )}
              </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-gray-200">
          ...
          {editMode ? (
            <div className="flex flex-col items-end gap-1">
              ...
              <span className="text-xs text-amber-500">ECOUNT must be updated manually after changes</span>
            </div>
          ) : (
            <span className="text-sm font-medium text-gray-800">{(order.freight??0) > 0 ? `₩${fmtNum(order.freight)}` : '—'}</span>
          )}
        </div>
        ...
        {!editMode && (() => {
          ...
          return (
            <div className="rounded-xl border border-gray-200 overflow-hidden">
              <div className="bg-gray-50 px-4 py-2.5 text-xs font-medium text-gray-500 border-b border-gray-100">Order Sending History</div>
              <table className="w-full text-xs">
                <thead className="bg-gray-50 text-gray-400 border-b border-gray-100"><tr><th className="px-4 py-2 text-left">Date/Time</th><th className="px-4 py-2 text-left">Recipient</th><th className="px-4 py-2 text-left">File</th></tr></thead>
                <tbody className="divide-y divide-gray-50">
                  {events.map((e,i) => (
                    <tr key={i} className={e.type === 'delivered' ? 'bg-green-50/50' : 'bg-white'}>
                      ...
                      <td className="px-4 py-2">{(e as any).to ?? <span className="text-green-600 font-medium">✓ Delivery Confirmed</span>}</td>
                      <td className="px-4 py-2">{e.type === 'send' && (e as any).fileUrl && <a href={(e as any).fileUrl} download={(e as any).fileName} className="text-[#014A99] underline hover:opacity-70">{(e as any).fileName ?? 'Download'}</a>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {!editMode && (() => {
          ...
          return (
            <div className="rounded-xl border border-gray-200 px-4 py-3 space-y-2">
              <p className="text-xs font-medium text-gray-500">Status Change History</p>
              {[...events].reverse().map((e,i) => (
                <div key={i} className="flex items-center gap-3 text-xs">
                  <span className={`px-2 py-0.5 rounded-full border text-xs font-medium ${STATUS_COLORS[e.value as OrderStatus] ?? 'bg-gray-100 text-gray-500'}`}>{statusLabel(e.value, ORDER_STATUS_LABEL)}</span>
        ...
        {!editMode && order.created_at && <p className="text-xs text-gray-300 text-right">Created: {toKST(order.created_at)}</p>}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={e => { if (e.target===e.currentTarget) setPendingStatus(null) }}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-semibold text-gray-900">Change Status</p><p className="text-sm text-gray-500 mt-0.5">Change the status?</p></div>
              </div>
              <div className="bg-gray-50 rounded-lg px-4 py-3 flex items-center gap-3 text-sm">
                <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full border ${STATUS_COLORS[order.status]}`}>{statusLabel(order.status, ORDER_STATUS_LABEL)}</span>
                ...
                <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full border ${STATUS_COLORS[pendingStatus]}`}>{statusLabel(pendingStatus, ORDER_STATUS_LABEL)}</span>
              </div>
              <div className="flex gap-2 justify-end">
                <button onClick={() => setPendingStatus(null)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 cursor-pointer">Cancel</button>
                <button onClick={() => { updateStatus(pendingStatus); setPendingStatus(null) }} className="px-4 py-2 rounded-lg text-sm font-semibold text-white cursor-pointer" style={{ backgroundColor:'#014A99' }}>Confirm</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={e => { if (e.target===e.currentTarget) { setShowFreightModal(false); setFreightInput('') } }}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-semibold text-gray-900">Mark as Delivered</p><p className="text-sm text-gray-500 mt-0.5">Enter freight cost to mark this order as delivered.</p></div>
              </div>
              <div>
                ...
                <input type="number" value={freightInput} onChange={e => setFreightInput(e.target.value)} autoFocus
                  onKeyDown={e => { if (e.key==='Enter') { const f=Number(freightInput); if(!f||f<=0){toast.error('Please enter a freight cost.');return} setShowFreightModal(false);setFreightInput('');updateStatus('납품',f) } }}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-[#014A99]" placeholder="e.g. 50000" min="1" />
              </div>
              <div className="flex gap-2 justify-end">
                <button onClick={() => { setShowFreightModal(false); setFreightInput('') }} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 cursor-pointer">Cancel</button>
                <button onClick={() => { const f=Number(freightInput); if(!f||f<=0){toast.error('Please enter a freight cost.');return} setShowFreightModal(false);setFreightInput('');updateStatus('납품',f) }}
                  className="px-4 py-2 rounded-lg text-sm font-semibold text-white cursor-pointer" style={{ backgroundColor:'#014A99' }}>Mark Delivered</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={e => { if (e.target===e.currentTarget) setShowDeleteConfirm(false) }}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-semibold text-gray-900">Delete Order</p><p className="text-sm text-gray-500">This cannot be undone.</p></div>
              </div>
              <div className="bg-gray-50 rounded-lg px-4 py-3 text-sm space-y-1">
                <p><span className="text-gray-400 w-16 inline-block">Vendor</span><span className="font-medium">{order.vendor}</span></p>
                <p><span className="text-gray-400 w-16 inline-block">Project</span>{order.project||'—'}</p>
              </div>
              <div className="flex gap-2 justify-end">
                <button onClick={() => setShowDeleteConfirm(false)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 cursor-pointer">Cancel</button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden max-h-[90vh] flex flex-col">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
                <div><h2 className="text-base font-bold text-gray-900">Send Order</h2><p className="text-xs text-gray-400 mt-0.5">Attach a PDF and send.</p></div>
              ...
              <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
                <div>
                  <p className="text-xs font-semibold text-gray-500 mb-2">Recipient</p>
                  <div className="flex items-center gap-2 bg-blue-50 rounded-lg px-3 py-2">
                    ...
                    <input type="email" value={recipientEmail} onChange={e => setRecipientEmail(e.target.value)} className="flex-1 bg-transparent text-sm text-gray-800 font-medium focus:outline-none min-w-0" placeholder="Recipient email" />
                  </div>
                </div>
                <div>
                  <p className="text-xs font-semibold text-gray-500 mb-2">Attachment</p>
                  <label className="flex items-center gap-3 border-2 border-dashed border-gray-200 rounded-xl px-4 py-3 cursor-pointer hover:border-[#014A99]/40 hover:bg-blue-50/30 transition-colors">
                    ...
                    {attachedFile ? <span className="text-sm text-[#014A99] font-medium truncate">{attachedFile.name}</span> : <span className="text-sm text-gray-400">Select a PDF file</span>}
                ...
                <div>
                  <p className="text-xs font-semibold text-gray-500 mb-2">Sender Signature</p>
                ...
                <div>
                  <p className="text-xs font-semibold text-gray-500 mb-2">Email Body</p>
                ...
                {sendResult === 'success' && <div className="bg-green-50 border border-green-200 rounded-lg px-4 py-3 text-sm text-green-700 flex items-center gap-2"><svg className="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>Order sent successfully.</div>}
                {sendResult === 'error' && <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-sm text-red-700">{sendError ?? 'An error occurred while sending.'}</div>}
              </div>
              <div className="px-6 py-4 border-t border-gray-100 flex gap-3 justify-end shrink-0">
                <button onClick={closeSendDialog} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 cursor-pointer">{sendResult==='success'?'Close':'Cancel'}</button>
                {sendResult !== 'success' && (
                  <button onClick={handleSendOrder} disabled={sending||!attachedFile} className="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold text-white disabled:opacity-40 cursor-pointer" style={{ backgroundColor:'#014A99' }}>
                    {sending ? <><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>Sending...</> : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>Send</>}
                  </button>
                )}
              </div>
            </div>
        ...
        {showSigManager && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden max-h-[85vh] flex flex-col">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
                <h2 className="text-base font-bold text-gray-900">Signature Settings</h2>
                  ...
                  <>
                    {signatures.map(sig => (
                      <div key={sig.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => startEditSig(sig)} className="text-xs text-gray-500 hover:text-[#014A99] cursor-pointer">Edit</button>
                          <button onClick={() => deleteSig(sig.id)} className="text-xs text-red-400 hover:text-red-600 cursor-pointer">Delete</button>
                        </div>
                      </div>
                    ))}
                    <button onClick={startAddSig} className="w-full py-2.5 rounded-lg text-sm font-medium text-[#014A99] border-2 border-dashed border-[#014A99]/30 hover:border-[#014A99]/60 hover:bg-blue-50/30 cursor-pointer">+ Add Signature</button>
                  </>
                ) : (
                  sigDraft && (
                    <div className="space-y-3">
                      {(['label','name','title','phone','email','web','address'] as const).map(k => (
                        <div key={k} className="flex items-center gap-3">
                          <label className="text-xs text-gray-400 w-16 shrink-0">{{label:'Label',name:'Name',title:'Title',phone:'Phone',email:'Email',web:'Website',address:'Address'}[k]}</label>
                      ...
                      <div className="flex gap-2 pt-2">
                        <button onClick={() => { setSigFormMode('none'); setSigDraft(null) }} className="flex-1 py-2 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">Cancel</button>
                        <button onClick={saveSigDraft} className="flex-1 py-2 text-sm font-semibold text-white rounded-lg cursor-pointer" style={{ backgroundColor:'#014A99' }}>Save</button>
                      </div>
                    </div>
                  )
                )}
      ...
      {type === 'pipe' && (
        <div id="print-area" ref={printRef} style={{ display: 'none' }}>
          <div style={{ padding: '20px', fontFamily: 'sans-serif', fontSize: '12px' }}>
            <h2 style={{ fontSize: '16px', marginBottom: '12px' }}>{order.project} Purchase Order</h2>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
              <thead><tr style={{ background: '#f5f5f5' }}>
                <th style={{ border: '1px solid #ddd', padding: '6px', textAlign: 'left' }}>No</th>
                <th style={{ border: '1px solid #ddd', padding: '6px', textAlign: 'left' }}>Item</th>
                <th style={{ border: '1px solid #ddd', padding: '6px', textAlign: 'left' }}>Spec</th>
                <th style={{ border: '1px solid #ddd', padding: '6px', textAlign: 'center' }}>Qty</th>
                <th style={{ border: '1px solid #ddd', padding: '6px', textAlign: 'left' }}>Internal Name</th>
              </tr></thead>
  ...
  return (
    <div className="rounded-xl border border-gray-200 overflow-hidden">
      <div className="bg-gray-50 px-4 py-2.5 text-xs font-medium text-gray-500 border-b border-gray-200 flex items-center justify-between">
        <span>Item List ({allItems.length})</span>
        {discountPct !== null && <span className="text-gray-400">Sale price = Negotiated price × {discountPct}%</span>}
      </div>
      {editMode ? (
        <>
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[500px]">
              <thead className="bg-gray-50 text-gray-500 text-xs border-b border-gray-100"><tr>
                ...
                <th className="px-4 py-2.5 text-left w-28">Spec</th>
                <th className="px-4 py-2.5 text-center w-16">Qty</th>
                <th className="px-4 py-2.5 text-left w-52">Internal Name / Spec</th>
                <th className="px-4 py-2.5 text-left w-32">Note</th>
                <th className="w-8" />
              </tr></thead>
              <tbody className="divide-y divide-gray-100">
                {allItems.map((item, i) => (
                  <tr key={i} className="bg-white">
                    ...
                    <td className="px-2 py-2"><input value={editData!.items[i]?.name??''} onChange={e => setItemField(i,'name',e.target.value)} className={INPUT_CLS} placeholder="Item Name" /></td>
                    <td className="px-2 py-2"><input value={editData!.items[i]?.spec??''} onChange={e => setItemField(i,'spec',e.target.value)} className={INPUT_CLS} placeholder="Spec" /></td>
                    ...
                    <td className="px-2 py-2"><input value={editData!.items[i]?.note??''} onChange={e => setItemField(i,'note',e.target.value)} className={INPUT_CLS} placeholder="Note" /></td>
          ...
          <div className="px-4 py-2.5 border-t border-gray-100">
            <button onClick={addItem} className="flex items-center gap-1.5 text-xs text-[#014A99] hover:underline cursor-pointer"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>Add Row</button>
          </div>
        </>
      ) : (
        <>
          {hasMultiMfr && (
            <div className="flex border-b border-gray-200 bg-white overflow-x-auto">
              <button onClick={() => setActiveMfrTab(null)} className={`px-4 py-2.5 text-xs font-medium border-b-2 -mb-px whitespace-nowrap transition-colors cursor-pointer ${!activeMfr ? 'border-[#014A99] text-[#014A99]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>All ({allItems.length})</button>
              {pipeMfrs.map(mfr => <button key={mfr} onClick={() => setActiveMfrTab(mfr)} className={`px-4 py-2.5 text-xs font-medium border-b-2 -mb-px whitespace-nowrap transition-colors cursor-pointer ${activeMfr===mfr ? 'border-[#014A99] text-[#014A99]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>{mfr} ({allItems.filter(it=>it.manufacturer===mfr).length})</button>)}
            </div>
          )}
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[600px]">
              <thead className="bg-gray-50 text-gray-500 text-xs border-b border-gray-100"><tr>
                <th className="px-4 py-2.5 text-left w-10">No</th>
                {showMfrCol && <th className="px-4 py-2.5 text-left w-24">Manufacturer</th>}
                <th className="px-4 py-2.5 text-left">Item Name</th>
                <th className="px-4 py-2.5 text-left w-28">Spec</th>
                <th className="px-4 py-2.5 text-center w-16">Qty</th>
                <th className="px-4 py-2.5 text-left w-52">Internal Name / Spec</th>
                <th className="px-4 py-2.5 text-right w-28">Unit Price</th>
                <th className="px-4 py-2.5 text-right w-32">Supply Amount</th>
                <th className="px-4 py-2.5 text-left w-32">Note</th>
              </tr></thead>
              <tbody className="divide-y divide-gray-100">
              ...
              {(() => {
                ...
                return <tfoot><tr className="bg-gray-50 border-t border-gray-200 font-semibold text-sm">
                  <td colSpan={showMfrCol ? 8 : 7} className="px-4 py-2.5 text-right text-gray-500 text-xs">Total</td>
          ...
          {!editMode && (
            <div className="border-t border-gray-100 grid grid-cols-3">
              <div className="px-4 py-3 text-center bg-blue-50/40 border-r border-blue-100">
                <p className="text-xs text-blue-400 font-medium mb-0.5">Sales</p>
                <p className="text-sm font-bold text-blue-700">{displaySaleAmount > 0 ? displaySaleAmount.toLocaleString() + ' KRW' : '—'}</p>
                {displaySaleAmount > 0 && <p className="text-xs text-blue-300 mt-0.5">(VAT incl. {Math.round(displaySaleAmount * 1.1).toLocaleString()} KRW)</p>}
              </div>
              <div className="px-4 py-3 text-center bg-red-50/30 border-r border-red-100">
                <p className="text-xs text-red-400 font-medium mb-0.5">Cost</p>
                {order.no_invoice ? (
                  <p className="text-sm text-gray-300 text-xs">No Invoice</p>
                ) : (order.purchase_amount ?? 0) > 0 ? (
                  <>
                    <p className="text-sm font-bold text-red-600">{((order.purchase_amount ?? 0) + (order.freight ?? 0)).toLocaleString()} KRW</p>
                    <p className="text-xs text-red-300 mt-0.5">(VAT incl. {Math.round(((order.purchase_amount ?? 0) + (order.freight ?? 0)) * 1.1).toLocaleString()} KRW)</p>
                  </>
                ) : <p className="text-sm text-gray-300 text-xs">Not yet purchased</p>}
              </div>
              <div className="px-4 py-3 text-center bg-green-50/40">
                <p className="text-xs text-green-500 font-medium mb-0.5">Profit</p>
                {order.no_invoice ? (
                  <p className="text-sm text-gray-300 text-xs">No Invoice</p>
                ) : (order.purchase_amount ?? 0) > 0 ? (() => {
                  ...
                  return <p className="text-sm font-bold text-green-700">{profit.toLocaleString()} KRW <span className="text-xs font-normal text-green-400">({margin}%)</span></p>
            ...
            return (
              <div className="px-4 py-3 border-t border-gray-100 flex items-center gap-3 flex-wrap">
                ...
                <button onClick={() => openSendDialog(mfrName)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white rounded-md transition-colors cursor-pointer" style={{ backgroundColor:'#014A99' }}>
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>Send Purchase Order
                </button>
                {order.status === '발주' && <span className="text-xs text-amber-600">Already sent</span>}
              </div>
            )
          })()}
        </>
  ...
  return (
    <div className="rounded-xl border border-gray-200 overflow-hidden">
      <div className="bg-gray-50 px-4 py-2.5 text-xs font-medium text-gray-500 border-b border-gray-200">
        <span>Item List ({regularItems.length})</span>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500 text-xs border-b border-gray-100"><tr>
            <th className="px-3 py-2.5 text-center w-10">No</th>
            <th className="px-3 py-2.5 text-center w-16">Item</th>
            <th className="px-3 py-2.5 text-right w-20">Width (mm)</th>
            <th className="px-3 py-2.5 text-right w-20">Height (mm)</th>
            <th className="px-3 py-2.5 text-right w-20">M/pc</th>
            <th className="px-3 py-2.5 text-right w-16">Qty</th>
            <th className="px-3 py-2.5 text-right w-24">Unit Price</th>
            <th className="px-3 py-2.5 text-right w-28">Supply Amount</th>
            <th className="px-3 py-2.5 text-left">Note</th>
            {editMode && <th className="w-8" />}
          </tr></thead>
          <tbody className="divide-y divide-gray-100">
            {regularItems.length === 0 && !editMode && <tr><td colSpan={9} className="px-4 py-6 text-center text-sm text-gray-400">No items</td></tr>}
            {editMode ? editRegular.map((item, i) => {
              ...
              return (
                <tr key={origIdx} className="bg-white">
                  ...
                  <td className="px-2 py-2"><select value={item.type} onChange={e => setItemField(origIdx,'type',e.target.value)} className={INPUT_CLS}><option value="입상">Riser</option><option value="벽체">Wall</option></select></td>
                  ...
                  <td className="px-2 py-2"><input value={item.note??''} onChange={e => setItemField(origIdx,'note',e.target.value)} className={INPUT_CLS} placeholder="Note" /></td>
              ...
              <tr key={i} className="bg-white">
                ...
                <td className="px-3 py-2.5 text-center"><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS_DUCT[item.type]??''}`}>{item.type === '입상' ? 'Riser' : item.type === '벽체' ? 'Wall' : item.type}</span></td>
            ...
            {editMode && <tr><td colSpan={10} className="px-3 py-2"><button onClick={addItem} className="text-xs text-[#014A99] hover:underline flex items-center gap-1 cursor-pointer"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>Add Item</button></td></tr>}
            {!editMode && totalAmount > 0 && <tr className="bg-gray-50 font-semibold"><td colSpan={7} className="px-3 py-2.5 text-right text-xs text-gray-500">Total</td><td className="px-3 py-2.5 text-right text-gray-800 tabular-nums">{totalAmount.toLocaleString('ko-KR')} KRW</td><td /></tr>}
          </tbody>
        </table>
      </div>
      ...
      {isProfireManufacturer(d.manufacturer) && (editMode || insulItems.length > 0) && (
        <div className="border-t border-orange-100">
          <div className="px-4 py-2.5 bg-orange-50/70"><span className="text-xs font-semibold text-orange-700">Insulation</span></div>
          <table className="w-full text-sm">
        ...
        return (
          <div className="border-t border-gray-100 grid grid-cols-3">
            <div className="px-4 py-3 text-center bg-blue-50/40 border-r border-blue-100">
              ...
              {order.no_invoice
                ? <p className="text-sm text-gray-300 text-xs">No Invoice</p>
                : <><p className="text-sm font-bold text-blue-700">{sale > 0 ? sale.toLocaleString() + ' KRW' : '—'}</p>
                  {sale > 0 && <p className="text-xs text-blue-300 mt-0.5">(VAT incl. {Math.round(sale * 1.1).toLocaleString()} KRW)</p>}</>
              }
            </div>
            <div className="px-4 py-3 text-center bg-red-50/30 border-r border-red-100">
              <p className="text-xs text-red-400 font-medium mb-0.5">Cost</p>
              {order.no_invoice ? (
                ...
                <>
                  <p className="text-sm font-bold text-red-600">{cost.toLocaleString()} KRW</p>
                  <p className="text-xs text-red-300 mt-0.5">(VAT incl. {Math.round(cost * 1.1).toLocaleString()} KRW)</p>
                </>
              ) : <p className="text-sm text-gray-300 text-xs">Not yet purchased</p>}
            </div>
            <div className="px-4 py-3 text-center bg-green-50/40">
              ...
              {order.no_invoice ? (
                ...
                ? <p className="text-sm font-bold text-green-700">{profit.toLocaleString()} KRW{margin != null && <span className="text-xs font-normal text-green-400 ml-1">({margin}%)</span>}</p>
                : <p className="text-sm text-gray-300 text-xs">—</p>}
            </div>
          </div>
        )
